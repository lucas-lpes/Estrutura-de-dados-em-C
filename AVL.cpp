...
arvore *RotacaoDir(arvore *a)
{
    ...
    {
        arvore *temp = a->esq;
        hd = descobrir_altura(temp->dir);
        he = descobrir_altura(temp->esq);
        if (hd > he)
        {
            a->esq = RotSimplesEsq(a->esq);
            a = RotSimplesDir(a);
        }
        else
            a = RotSimplesDir(a);
